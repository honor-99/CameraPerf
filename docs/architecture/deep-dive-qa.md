...
> 这篇文章收集了[《从 Trace 到洞察：CameraPerf AI Agent 的 Harness Engineering 实战》](./deep-dive.md)发布后收到的技术问题，以问答形式展开讨论。
...
Claude Code Skills 和 CameraPerf YAML Skills 解决的是不同阶段的问题：

```
开发阶段（我写代码时）:
  Claude Code + Skills/Hooks → 帮我开发 CameraPerf

运行阶段（用户分析 trace 时）:
  CameraPerf Backend + YAML Skills → 帮用户分析性能数据
```

Claude Code 的 Skill 运行在开发者的终端里，是 CLI 工具的扩展。CameraPerf 的 YAML Skill 运行在 Express 后端的 Skill Engine 中，是 Agent 在运行时通过 MCP 工具 `invoke_skill` 调用的分析单元。两者的执行环境、调用方式、数据流完全不同。
...
| 维度 | Claude Code scripts/ | CameraPerf YAML Skill |
|------|---------------------|--------------------------|
...
### CameraPerf 的做法：约束强度光谱

CameraPerf 没有在 Agent 和 Workflow 之间二选一，而是对不同分析阶段设定了不同的约束强度（详见 Q2）。这里从「能力边界」的角度重新审视这个设计：
...
同一个模型，给它 `scrolling_analysis` Skill 的 L2 结构化帧数据 vs 让它自己写 SQL 查原始表，分析质量差距非常显著。Agent 的上限由你给它的数据工具决定。CameraPerf 用 164 个 YAML Skill 封装了领域专家的查询逻辑，Agent 通过 `invoke_skill` 拿到的是处理过的、结构化的分析数据，而不是原始的百万行 trace 事件。
...
Agent 的 findings 中存在相当比例的问题（浅层归因、假阳性、遗漏关键步骤）。单纯依赖模型自我纠错效果有限。CameraPerf 用多层验证 + 外部纠错 prompt 来闭环：
...
**直说：在数据收集阶段，CameraPerf 的滑动分析就是一个 Workflow。** Strategy 文件就是 SOP，它把领域专家的分析经验编码成了确定性步骤。这是故意的。
...
**所以正确的理解是：CameraPerf = 「SOP 驱动的数据收集 + Agent 驱动的推理归因」。**
...
**提问背景：** Agent 的架构有过不同的设计和演进——从最初的 ReAct 架构，到 LangGraph 的节点式架构，不同的 Agent 架构设计会给它带来怎样的影响？在搭建自己的业务 Agent 时，是否需要考虑架构对 Agent 性能的影响？例如 CameraPerf 是在 Claude Agent SDK 的基础上基于业务理解加入了不同的 Skill 加载模式。
...
### CameraPerf 选择 SDK 原生 + 自建约束层的原因

CameraPerf 的架构是 **Claude Agent SDK（SDK 原生）+ 三层约束（Strategy/Planning Gate/Verifier）**：

```
Claude Agent SDK 提供:                CameraPerf 自建:
...
以下是 CameraPerf 基于业务理解做的架构改进，每一个都直接对应一个业务问题：

**1. 条件化工具加载——减少 Agent 的决策空间**

CameraPerf 的 MCP 工具总数最多 20 个（9 个 always-on + 11 个条件注入），但一次分析只注入其中的子集：
...
当用户问「这个 trace 的帧率是多少」时，不需要走完整的 Planning → Skill → Verification 管线。CameraPerf 的 `ClaudeRuntime` 在入口处做复杂度分类：
...
**架构选择是业务问题，不是技术问题。ReAct/LangGraph/SDK 只是控制流的不同实现方式——真正影响 Agent 性能的是你在控制流之上搭建的约束层。CameraPerf 选择 SDK 原生不是因为它最先进，而是因为性能分析的根因路径不可预测，预定义 DAG 不如让 Agent 在约束框架内自主探索。**
...
### CameraPerf 的做法：三层信号、分工明确

CameraPerf 的场景识别不是靠单一信号源，而是三层信号配合，每层解决不同的问题：
...
关键词匹配的确有边界——当用户说「这个 app 为什么慢」时，关键词无法确定是启动慢还是滑动慢。CameraPerf 的处理方式是：**匹配不到就 fallback 到 `general` 场景**，让 Agent 在 general 策略的路由决策树中自主选择方向。
...
Trace 采集配置不同，可用的数据维度也不同。有的 trace 没有 GPU frequency 数据，有的没有 thermal zone。CameraPerf 在分析开始前探测 18 个数据维度的可用性：
...
提问者说「关键词匹配会导致筛选结果过大或过小」。CameraPerf 的经验是：
...
LLM 的分类在 CameraPerf 中不是 Layer 1，而是 `general` 场景中 Agent 的自主路由——这时 Agent 已经拿到了 trace 数据，可以结合数据做更准确的判断。

**3. 日志还原：适合做 Layer 2 的补充信号**

日志能告诉你「trace 中有什么」（有无滑动事件、有无 ANR），但不能告诉你「用户关心什么」。CameraPerf 的数据完整性探测就是这个角色——它不参与场景分类，但为 Agent 提供数据可用性信息。
...
**提问背景：** 目前线上 Skill 运行的时候发现，AI 自主探索、下钻根因容易跑偏，给出不正确的结果。比如 CameraPerf 博客中提到的例子——在前期定位到 RenderThread 被 Binder 阻塞后（基于确定性步骤），后面的多个假设形成和验证是纯 AI 发挥吗，还是说针对 Binder 阻塞我们可能给 AI 一些常见的原因作为引导让它自己排查？

### 先回答核心问题：不是纯 AI 发挥，也不是写死引导

CameraPerf 的做法是**结构化推理框架 + 按需知识注入**：
...
CameraPerf 提供了 `submit_hypothesis` 和 `resolve_hypothesis` 两个 MCP 工具，不是让 Agent 在内心独白中隐式推理，而是**强制外显化**：
...
CameraPerf 还在 Strategy 文件中提供了**条件化的深钻建议表**，这也是一种引导：
...
在数据工具（`execute_sql` / `invoke_skill`）成功返回的**前几次**调用中，CameraPerf 在结果末尾附加一段推理提示：
...
**提问背景：** 大模型 Agent 的输出质量很大程度取决于 system prompt 的设计。CameraPerf 在每次分析时是如何构造 prompt 的？不同场景、不同轮次之间 prompt 有什么变化？token 预算怎么控制？

### 总体设计：四层分层拼接 + 缓存优化

CameraPerf 的 system prompt 不是一个静态字符串，而是由 `buildSystemPrompt()` 函数（`claudeSystemPrompt.ts:260`）在每次 SDK 查询前**动态拼接**的。拼接遵循一个核心原则：
...
不是所有查询都需要完整的 4500-token prompt。当用户问事实性问题（如「帧率是多少」）时，CameraPerf 使用精简的 quick prompt：
...
## Q8：CameraPerf 有哪些 Skill？

**提问背景：** CameraPerf 的分析能力由 YAML Skill 承载。完整的 Skill 清单可以帮助理解系统的分析覆盖范围。
