...
analyze(query, sessionId, traceId)
    ...
    │     ├─ classifyScene(query) → camera_preview/video_record/general/...
...
| 类型 | 数量 | 位置 | 说明 |
|------|------|------|------|
| **Atomic** | 23 | `skills/atomic/` | 单一检测能力 (如 VSync 检测、CPU 频率分析) |
| **Composite** | 11 | `skills/composite/` | 组合分析 (如 scrolling_analysis = 多个 atomic 的编排) |
| **Deep** | 2 | `skills/deep/` | 深度分析 (cpu_profiling, callstack_analysis) |
| **Pipeline** | 0 | `skills/pipelines/` | 渲染管线检测 + 教学 (Camera 专用管线类型) |
| **Module** | 7 | `skills/modules/` | 模块分析 (app/framework/hardware/kernel) |
| **总计** | **43** | | |
...
CameraPerf 支持 Android Camera 渲染管线的自动识别和教学：
...
专注 Camera 领域类型，每个场景有对应的 `.strategy.md` 文件：
...
前端根据 `display` 配置**自动渲染**——不需要为每个 Skill 写专门的 UI。这就是为什么可以有 43 个 Skills 而前端代码量仍然可控。
